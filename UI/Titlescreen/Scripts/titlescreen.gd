...
func _process(delta):
	...
	if Input.is_action_just_pressed("ui_select_1") or Input.is_action_just_pressed("ui_select_2"):
		...
		for b in dec:
			if b.get_parent().name == "Brawl Btn":
				b.get_parent().emit_signal("pressed")
			elif b.get_parent().name == "Settings Btn":
				b.get_parent().emit_signal("pressed")
			elif b.get_parent().name == "Leave Btn":
				b.get_parent().emit_signal("pressed")
...
func play():
	getpointer.global_position = Vector2(1490, 375)

func options():
	getpointer.global_position = Vector2(1490, 510)

func exit():
	getpointer.global_position = Vector2(1490, 645)
